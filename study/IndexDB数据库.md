...
**LocalStorage**：依据浏览器不同在2.5MB到10MB之间，而且不提供搜索功能，不能建立自定义的索引。
...
1. **键值对储存**。indexedDB内部采用对象仓库（object store）存放数据。所有类型的数据都可以直接存入，包括JavaScript对象。对象仓库中，数据以"键值对"的形式保存，每一个数据记录都有对应的主键，主键是独一无二的，不能有重复，否则会抛出一个错误。
...
5. **事务**
   ...
   - 数据记录的读写和删改，都要通过事务完成。事务对象提供`error`、`abort`和`complete`和三个事件，用来监听操作结果。
...
`indexedDB.open()`返回一个IDBRequest对象。这个对象通过三种事件`error`、`success`、`upgradeneeded`,处理打开数据库的操作结果。
...
如果数据记录里面没有合适做为主键的属性，那么可以让IndexedDB自动生成主键。
...
新增数据指的是向对象仓库`（object store）`写入数据记录。这主要通过事务`(transaction)`完成。
...
上述代码中，写入数据需要新建一个事务`（transaction）`。新建时必需置顶表格名称和操作模式（”只读`（readonly）`“或”读写`（readwrite）`“）。新建事务以后，通过`IDBTransaction.objectStore(name)`方法，拿到 `IDBObjectStore`对象，再通过表格对象的`add()` 方法，向表格写入一条记录。
...
`indexedDB.cmp()`方法比较两个值是否为indexedDB的相同的主键。它返回一个整数，表示比较的结果：`0`表示相同，`1`表示第一个主键大于第二个主键，`-1`表示第一个主键小于第二个主键。
...
- `IDBRequest.result`：返回请求的结果。如果请求失败、结果不可用，读取改属性会报错。

- `IDBRequest.error`：请求失败时，返回错误对象。

- `IDBRequest.source`：返回请求的来源（比如索引对象或ObjectStore）。
...
- `IDBDatabase.objectStoreNames`：DOMStringList 对象（字符串的集合），包含当前数据的所有 object store 的名字。即当前数据库内所有表。
...
```javascript
let t = db.transaction(['items'], 'readwrite');
```
...
let range = IDBKeyRange.bound('B', 'D');
...
```javascript
let keyRangeValue = IDBKeyRange.bound('A', 'K', false, false);
